    ...
    contract Token is IERC20, IERC1404 {
       ...
       function removeFromEscrow(uint _amount) public onlyValidShareHolder returns(uint totalInEscrow) {
        ...
        function recordByCat(address _holder, Sharing.TokenCat _sitCat, bytes32 _recordId) public view returns (bytes32 recordId, uint256 amount, uint256 dateAdded, bool isMovedToTradable, bool isWithdrawn) {
            (recordId, amount, dateAdded, isMovedToTradable, isWithdrawn) = TokenFunc._getRecordByCat_(tokenFunc, _holder, _sitCat, _recordId);
        }
        ...
        function preloadToken(address _holder, uint _lien, uint _upfront, uint _tradable, bytes32 _idLien, bytes32 _idUpfront, bytes memory _data) public returns (string memory success) {
            return TokenScheduler._preloadToken_(tokenFunc, uGranularity, _holder, _lien, _upfront, _tradable, _idLien, _idUpfront, _data);
        }
